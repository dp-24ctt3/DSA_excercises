...
void removeTail(List *&L)
{
    ...
    if (L->p_head != nullptr) {
        ...
        while (curNode->p_next != L->p_tail) curNode = curNode->p_next;
        delete L->p_tail;
        L->p_tail = curNode;
...
void removeBefore(List *&L, int val)
{
    ...
    while (curNode->p_next->p_next != nullptr){
        if (curNode->p_next->p_next->key == val) break;
        curNode = curNode->p_next;
    }
    if (curNode->p_next->p_next != nullptr) {
        NODE* del = curNode->p_next;
        curNode->p_next = curNode->p_next->p_next;
        delete del;
    }
    return;
}
...
void removeAfter(List *&L, int val)
{
    ...
    if (curNode->p_next != nullptr){
        if (curNode->p_next == L->p_tail) removeTail(L);
        else{
            NODE* del = curNode->p_next;
            curNode->p_next = curNode->p_next->p_next;
            delete del;
...
void removePos(List *&L, int data, int pos)
{
    ...
    if (curNode!= nullptr){
        NODE* del = curNode->p_next;
        if (curNode->p_next == nullptr) L->p_tail = curNode;
        curNode->p_next = curNode->p_next->p_next;
        delete del;
    }
    return;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    NODE* curNode = L->p_head;
    while (curNode != nullptr){
        if (curNode->p_next != nullptr && curNode->p_next->key == val) break;
        curNode = curNode->p_next;
    }
    if (curNode != nullptr){
        NODE* p = createNode(data);
...
void removeDuplicate(List *&L)
{
    ...
    if (L->p_head != nullptr){
        for (NODE* i = L->p_head; i != nullptr; i = i->p_next){
            NODE* j = i;
            while (j != nullptr){
                if (j->p_next != nullptr && i->key == j->p_next->key){
                    NODE* del = j->p_next;
...
bool removeElement(List *&L, int key)
{
    ...
    NODE* curNode = L->p_head;
    while (curNode != nullptr){
        if (curNode->p_next != nullptr && curNode->p_next->key == key){
            NODE* del = curNode->p_next;
            if (curNode->p_next->p_next == nullptr) L->p_tail = curNode;
            curNode->p_next = curNode->p_next->p_next;
            delete del;
        }
        else{
